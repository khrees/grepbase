...
export function isPathSafe(path: string): boolean {
    ...
    const allowedDirs = ['src/', 'lib/', 'services/', 'components/', 'app/', 'hooks/', 'types/', 'db/'];
    const hasAllowedPrefix = allowedDirs.some(dir => normalized.startsWith(dir) || normalized.includes(`/${dir}`));
    ...
    const hasCodeExtension = CODE_EXTENSIONS.some(ext => normalized.endsWith(ext));
    const isCommonProjectFile = /README|LICENSE|package\.json|tsconfig|\.md$/i.test(normalized);
...
export function extractFileReferences(question: string): string[] {
    ...
    while ((match = simplePattern.exec(question)) !== null) {
        const path = match[1];
        if (references.length < 5 && !references.includes(path) && !references.some(ref => ref.endsWith('/' + path))) {
            references.push(path);
        }
    }
 ...
 */
export function detectPromptInjection(question: string): { isInjected: boolean; reason?: string } {
    const questionLower = question.toLowerCase();

    const injectionPatterns = [
        { regex: /ignore\s+(all\s+)?(previous|earlier|past|prior|before|preceding)/i, reason: 'Ignore instructions attempt' },
        { regex: /disregard\s+(all\s+)?(previous|earlier|past|prior)/i, reason: 'Disregard instructions attempt' },
        { regex: /act\s+as\s+(a|an|the)\s+/i, reason: 'Persona impersonation attempt' },
        { regex: /you\s+(are|be)\s+(a|an|the)\s+/i, reason: 'System prompt exposure attempt' },
        { regex: /output\s+(raw|system|internal|debug|developer|config|environment)/i, reason: 'Internal data exposure attempt' },
        { regex: /show\s+(me|the)\s+(system|config|environment|secret|key|token|password|credential)/i, reason: 'Secret exposure attempt' },
        { regex: /leak\s+(system|prompt|instructions?|config)/i, reason: 'Prompt leak attempt' },
        { regex: /bypass\s+(system|security|filter|protection)/i, reason: 'Bypass attempt' },
        { regex: /override\s+(system|policy|rule|instruction)/i, reason: 'Override attempt' },
        { regex: /replace\s+(your|the)\s+(system|prompt|instruction)/i, reason: 'Instruction replacement attempt' },
        { regex: /don['`](s|t)\s+(follow|obey|listen|comply|adhere)/i, reason: 'Direct non-compliance attempt' },
        { regex: / disregard (all of|my|the|your own) /i, reason: 'Disregard instruction attempt' },
        { regex: /prompt injection/i, reason: 'Explicit injection mention' },
        { regex: /system prompt/i, reason: 'System prompt inquiry' },
        { regex: /what (are|is) (you|your) (system|inner|base) (prompt|instruction)/i, reason: 'System prompt discovery attempt' },
        { regex: /how (are|is) (you|your) (prompt|instruction)/i, reason: 'Prompt structure inquiry' },
        { regex: /ignore all instructions? ever given to (you|u)/i, reason: 'Complete instruction override' },
        { regex: /you are an ai language model/i, reason: 'Self-identity change attempt' },
        { regex: /output the (full|entire) (system|internal) (prompt|instructions?)/i, reason: 'Full prompt extraction' },
    ];
 ...
 */
async function getExplorationCacheKey(repoId: string, commitSha: string | null, question: string): Promise<string> {
 ...
 */
export function cleanFileContent(content: string, maxLines: number = 100): string {
...
export async function exploreForQuestion(
    ...
    try {
        ...
        if (!sha) {
            ...
            const latestCommit = await db.select()
                .from(commits)
            ...
            const commitResult = await db.select()
                .from(commits)
        ...
        if (!commitId || !sha) {
            autoExplorerLogger.warn({ repoId, commitSha }, 'Could not resolve commit or SHA');
            return { context: '' };
        }
        ...
        const words = questionLower
            .replace(/[^\w\s-]/g, ' ')
            .split(/\s+/)
            .filter(w => w.length > 3 && !['what', 'where', 'when', 'how', 'does', 'done', 'implement', 'work', 'code', 'file', 'repo', 'repository', 'project', 'here', 'explain', 'understand'].includes(w));
        ...
        const scoredFiles = allFiles.map(file => {
            ...
            for (const ref of fileReferences) {
                ...
                if (pathLower === refLower) {
                    score += 100;
                } else if (pathLower.endsWith('/' + refLower) || fileName === refLower) {
                    score += 80;
                }
            }
            ...
            if (questionLower.includes('auth') && (pathLower.includes('auth') || pathLower.includes('session') || pathLower.includes('login') || pathLower.includes('credential') || pathLower.includes('token') || pathLower.includes('security'))) {
                score += 20;
            }
            if (questionLower.includes('db') || questionLower.includes('database') || questionLower.includes('schema') || questionLower.includes('drizzle')) {
                if (pathLower.includes('db') || pathLower.includes('schema') || pathLower.includes('query') || pathLower.includes('model') || pathLower.includes('migration')) {
                    score += 20;
                }
            }
            if (questionLower.includes('config') || questionLower.includes('env') || questionLower.includes('setting')) {
                if (pathLower.includes('config') || pathLower.includes('env') || pathLower.includes('const') || pathLower.includes('setting')) {
                    score += 20;
                }
            }
        ...
        if (matchedFiles.length === 0) {
            autoExplorerLogger.info({ question }, 'No matching files found for exploration');
            return { context: '' };
        }

        // 7. Fetch content for these matched files
        const exploredFilesContent: { path: string; content: string; language: string }[] = [];

        // Get repo details for fetching from GitHub if needed
        const repoResult = await db.select()
            .from(repositories)
        ...
        for (const file of matchedFiles) {
            ...
            const dbFile = await db.select({ content: files.content })
                .from(files)
            ...
            if (dbFile.length > 0 && dbFile[0].content) {
                ...
                // Fetch from GitHub
                try {
                    const fetched = await fetchFileContent(repo.owner, repo.name, sha, file.path);
                    if (fetched) {
                        content = fetched;
                    ...
                    autoExplorerLogger.error({ err, path: file.path }, 'Failed to fetch file content during exploration');
                }
            }
...
export function getExplorationGuidance(question: string): string | null {
    ...
    if (questionLower.includes('config') || questionLower.includes('setting') || questionLower.includes('env')) {
        return 'Check: src/lib/constants.ts, src/lib/platform/context.ts, .env.example';
    }

    if (questionLower.includes('database') || questionLower.includes('db') || questionLower.includes('schema')) {
        return 'Check: src/db/index.ts, src/db/schema.ts, migrations/';
    }

    if (questionLower.includes('api') || questionLower.includes('endpoint') || questionLower.includes('route')) {
